...
const burningComet = {
  ...
  title: 'Burning Comet',
  description:
    'Deal more than 3500 damage with Arcane Comet (rune). (ARAM: 4000 damage, Twisted Treeline: 3000 damage).',
  svgPath:
  ...
  obtainedCheck: ({ match, trophiesCategory }) => {
    if (trophiesCategory === ARAM) {
      return match.participant.stats.perk0 === 8229 && match.participant.stats.perk0Var1 >= 4000;
    }
    if (trophiesCategory === TWISTED_TREELINE) {
      return match.participant.stats.perk0 === 8229 && match.participant.stats.perk0Var1 >= 3000;
    }

    return match.participant.stats.perk0 === 8229 && match.participant.stats.perk0Var1 >= 3500;
  },
  context: 'matches',
  attribute: 'power',
  runeId: 8229
