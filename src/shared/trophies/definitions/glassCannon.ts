...
const glassCannon = {
  ...
  title: 'Glass Cannon',
  description:
    'Have most damage dealt to champions while taking the least damage. (Twisted Treeline: 20% more damage than the next player while taking least damage).',
  svgPath:
    ...
    if (trophiesCategory === TWISTED_TREELINE)
      return (
        match.participant.stats.totalDamageDealtToChampions >=
          1.2 * match.participant.stats.others.maxTotalDamageDealtToChampions &&
