    ...
    if (trophiesCategory === TWISTED_TREELINE)
      return (
        match.participant.stats.kills + match.participant.stats.assists >=
          match.maxKillParticipation &&
        match.participant.stats.totalHeal >= 1.2 * match.participant.stats.others.maxTotalHeal
      );

    return (
      match.participant.stats.kills + match.participant.stats.assists >=
        match.maxKillParticipation && match.participant.stats.totalHeal >= match.maxTotalHeal
    );
  },
  context: 'matches',
