...
const deadlyVenom = {
  ...
  title: 'Deadly Venom',
  description:
    'Deal at least 25% more damage to champions than the next player. (from 50%: Tuors Axe) (Twisted Treeline: Deal at least 40% more damage, from 70%: Tuors Axe).',
  svgPath:
    ...
    if (trophiesCategory === TWISTED_TREELINE)
      return (
        match.participant.stats.totalDamageDealtToChampions >=
          1.4 * match.participant.stats.others.maxTotalDamageDealtToChampions &&
        match.participant.stats.totalDamageDealtToChampions <
